...
#pragma omp parallel for num_threads(4)
    ...
    void serve(const PSFData<ParamInit>::Request &request,
        ...
        // no response result
        Key k = get<0>(request);
        PS_VLOG(3) << "Server perform ParamInit for " << k;
        ...
        PS_VLOG(3) << "Server get all params for ParamInit:" << k << " " << len
                   << " " << width << " " << init_type << " " << init_a << " "
                   << init_b << " " << seed << " " << otype;
        ...
        auto &value_set_ =
            *const_cast<typename tmap::mapped_type &>(iter->second);
        auto write_lock = value_set_.write_guard();
        PS_VLOG(3) << "Server start to init " << k << " with " << init_type;
        size_t n_threads = (value_set_.size() >> 25) + 1;
        if (n_threads > 16)
            n_threads = 16;
        if (init_type == InitType::Constant) {
            float filled_value = static_cast<float>(init_a);
            PS_VLOG(3) << "Server needs to init value set size"
                       << value_set_.size() << " with " << filled_value;
...
#pragma omp parallel num_threads(n_threads)
        ...
        PS_VLOG(3) << "Server inited " << k << " with " << init_type;
    }
