...
public:
    ...
    void pinMemory(void *ptr, size_t size) {
        Postoffice::Get()->van()->PinMemory(ptr, size, false);
    }
    ...
    void ParameterInit(const int name, InitType init_type, double init_a,
        ...
        for (size_t i = 0; i < meta.keys.size(); i++) {
            ...
            PS_VLOG(3) << "worker init params: " << meta.keys[i] << " "
                       << meta.ptype << " " << meta.part[i] << " " << meta.width
                       << " " << init_type << " " << init_a << " " << init_b
                       << " " << seed << " " << otype;
            meta.ts.push_back(_kvworker.RequestForParamInit(request, cb));
        }
    }
    ...
    void PReduceGetPartner(Key key, int rank, size_t required_worker_num,
                           float wait_time, int *result) {
        PSFData<kPReduceGetPartner>::Request request(
            key, rank, required_worker_num, wait_time);
    ...
    */
    ...
    void pushEmbedding(int name, const SArray<uint64_t> &rows,
        ...
        for (size_t i = 0; i < meta.keys.size(); i++) {
            ...
            meta.ts.push_back(_kvworker.RequestForPush(request, cb));
            cur_len += meta.part[i];
        }
    }
    ...
    void pushSyncEmbedding(int name, const SArray<uint64_t> &rows,
        ...
        for (size_t i = 0; i < meta.keys.size(); i++) {
            ...
            auto cb = std::bind(closure, std::placeholders::_1, start);
            meta.ts.push_back(_kvworker.RequestForPushSync(request, cb));
            cur_len += meta.part[i];
        }
    }
};
