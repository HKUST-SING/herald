...
inline Node::Role GetRoleFromEnv() {
    auto val = CHECK_NOTNULL(Environment::Get()->find("DMLC_ROLE"));
    std::string role_str(val);
    auto role = GetRole(role_str);
    return role;
}
...
inline void _StartPS(int customer_id, Node::Role role, int rank,
    ...
    if (role == Node::WORKER) {
        Postoffice::GetWorker(instance_idx)
            ->Start(customer_id, role, do_barrier, argv0);
...
inline void _StartPSGroup(int customer_id, std::vector<int> worker_ranks,
    ...
    for (size_t i = 0; i < worker_ranks.size(); ++i) {
        threads.emplace_back(_StartPS, customer_id, Node::WORKER,
                             worker_ranks[i], do_barrier, argv0, i);
        PS_VLOG(1) << "Postoffice worker rank " << worker_ranks[i]
                   << " started.";
    }
    for (size_t i = 0; i < server_ranks.size(); ++i) {
        threads.emplace_back(_StartPS, customer_id, Node::SERVER,
                             server_ranks[i], do_barrier, argv0, i);
        PS_VLOG(1) << "Postoffice server rank " << server_ranks[i]
                   << " started.";
    }
    for (auto &t : threads) {
        t.join();
 ...
 */
inline void Start(int customer_id, int rank = -1, bool do_barrier = true,
                  const char *argv0 = nullptr) {
    auto val = Environment::Get()->find("DMLC_GROUP_SIZE");
    int group_size = val ? atoi(val) : 1;
    auto role = GetRoleFromEnv();
    ...
    if (group_size == 1 || role == Node::SCHEDULER) {
        ...
        _StartPSGroup(customer_id, worker_ranks, server_ranks, do_barrier,
                      argv0);
    }
}
...
inline void _Finalize(int customer_id, Node::Role role,
    ...
    if (role == Node::WORKER) {
        ...
        // Joint PS: one worker, one server
        std::thread thread_s(&Postoffice::Finalize,
                             Postoffice::GetServer(index), customer_id,
                             do_barrier);
        PS_VLOG(1) << "Finalize Postoffice server.";

        std::thread thread_w(&Postoffice::Finalize,
                             Postoffice::GetWorker(index), customer_id,
                             do_barrier);
...
inline void _FinalizeGroup(int customer_id, Node::Role role, int group_size,
    ...
    if (role == Node::WORKER) {
        for (int i = 0; i < group_size; ++i) {
            threads.emplace_back(&Postoffice::Finalize,
                                 Postoffice::GetWorker(i), customer_id,
                                 do_barrier);
    ...
    if (role == Node::SERVER) {
        for (int i = 0; i < group_size; ++i) {
            threads.emplace_back(&Postoffice::Finalize,
                                 Postoffice::GetServer(i), customer_id,
                                 do_barrier);
 ...
 * true.
 */
inline void Finalize(int customer_id,
...
inline void StartAsync(int customer_id, const char *argv0 = nullptr) {
    auto role = GetRoleFromEnv();
    Postoffice::Get()->Start(customer_id, role, false, argv0);
}
