...
#define LOG_FATAL dmlc::LogMessageFatal(__FILE__, __LINE__).stream()
...
// log thread
#define LOG_T(tid) dmlc::LogMessage(__FILE__, __LINE__, tid)
...
public:
    LogMessage(const char *file, int line, int tid = 0) :
#ifdef __ANDROID__
        log_stream_(std::cout)
#else
        log_stream_(std::cerr), file(file), line(line), tid(tid)
#endif
    {
    }
    ~LogMessage() {
        // dump
        log_stream_ << "[" << hostname() << ":" << pretty_date_.HumanDate();
        if (tid != 0) {
            log_stream_ << ":TID=" << tid;
        }
        log_stream_ << "] ";
