...
protected:
    ...
    void Connect(const Node &node) override {
        ...
        // LOG(INFO) << "Connect from " << my_node_.DebugString() << " to " << node.DebugString();
    ...
    int SendMsg(const Message &msg) override {
        // LOG(INFO) << "Call RDMA SendMsg " << my_node_.DebugString();
        ...
        size_t meta_len = meta.ByteSizeLong();
        size_t data_len = msg.data_size();
        size_t total_len = meta_len + data_len;
        // LOG(INFO) << "meta len: " << meta_len << ", data_len: " << data_len;
...
private:
            ...
            for (int i = 0; i < ne; ++i) {
                ...
                case IBV_WC_RECV: {
                    ...
                    if (imm == kRendezvousStart) {
                        ...
                        // LOG(INFO) << "RDMA send msg with kRendezvousStart response: " << my_node_.ShortDebugString();
                        ...
                        *tmp = msg_buf; // write the addr of msg_buf into the mr
                        ...
                        // LOG(INFO) << "RDMA send msg with kRendezvousReply response: " << my_node_.ShortDebugString();
    ...
    void OnConnectRequest(struct rdma_cm_event *event) {
        ...
        // LOG(INFO) << "Try to accept connection from " << remote_ctx->node;
    ...
    void OnConnected(struct rdma_cm_event *event) {
        ...
        endpoint->cv.notify_all();
        // LOG(INFO) << "Accepted connection from " << endpoint->node_id;
    }
