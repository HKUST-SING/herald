...
struct RawMeta {
  ...
  // psftype of message
  PsfType psftype;
  // data number in request message
  int req_data_num;
  // the size of each data number in request message
  int req_data_len[kMaxDataNum];
  // data number in response message
  int resp_data_num;
  // the size of each data number in responce message
  int resp_data_len[kMaxDataNum];
  // local data buffer address 
  uint64_t local_data_addr;
  // local data buffer length
  int local_data_buf_len;
  // local option (used by remote side to echo local data addr)
  uint64_t local_option;
  // version bound
  uint64_t version_bound;
