...
void Van::ProcessBarrierCommand(Message *msg) {
    ...
    if (msg->meta.request) {
        ...
        if (barrier_count_[group]
            ...
            // first notify all servers
            std::set<int> notified = {};
            if (group & kServerGroup) {
            ...
            for (int r : Postoffice::Get()->GetNodeIDs(group)) {
                int recver_id = r;
                // notify other nodes
                if (std::find(notified.begin(), notified.end(), recver_id)
                    != notified.end()) {
                        continue;
                }
                if (shared_node_mapping_.find(r)
...
void Van::ProcessDataMsg(Message *msg) {
    ...
    auto *obj = Postoffice::Get()->GetCustomer(app_id, customer_id, 10);
    if (!obj) {
        PS_VLOG(1) << "Current number of customers: " << Postoffice::Get()->NumCustomers();
    }
