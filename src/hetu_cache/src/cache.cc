...
wait_t CacheBase::embeddingUpdateWithPushKeys(py::array_t<cache_key_t> _keys,
                                              py::array_t<cache_key_t> _push_keys,
                                              py::array_t<embed_t> _grads) {
    PYTHON_CHECK_ARRAY(_keys);
    PYTHON_CHECK_ARRAY(_push_keys);
    PYTHON_CHECK_ARRAY(_grads);
    auto grads = _grads.data();
    size_t num_keys = _keys.size();
    size_t num_push_keys = _push_keys.size();
    assert(size_t(_grads.size()) == num_keys * width_);
    SArray<cache_key_t> keys(_keys.mutable_data(), num_keys);
    SArray<cache_key_t> push_keys(_push_keys.mutable_data(), num_push_keys);
    return ThreadPool::Get()->Enqueue(&CacheBase::_embeddingUpdateWithPushKeys, this,
                                      keys, push_keys, grads);
}

wait_t CacheBase::embeddingUpdateWithPushKeysNpRaw(uint64_t _keys, py::array_t<cache_key_t> _push_keys, uint64_t grads,
                                     size_t num_keys) {
    float *keys = reinterpret_cast<float *>(_keys);
    SArray<cache_key_t> intkeys(num_keys);
    for (size_t i = 0; i < num_keys; i++)
...
wait_t CacheBase::embeddingUpdateWithPushKeysRaw(uint64_t _keys, uint64_t _push_keys, uint64_t grads,
    ...
    float *push_keys = reinterpret_cast<float *>(_push_keys);
    SArray<cache_key_t> intpushkeys(num_push_keys);
    for (size_t i = 0; i < num_push_keys; i++)
        intpushkeys[i] = (cache_key_t)push_keys[i];
...
void CacheBase::_embeddingUpdateWithPushKeys(SArray<cache_key_t> keys,
    ...
    size_t miss_cnt = 0, evict_cnt = evict_.size();
    vector<EmbeddingPT> should_push;
    vector<size_t> shoud_push_indexes;
    vector<EmbeddingPT> evict = std::move(evict_);
    for (size_t _i = 0; _i < keys.size(); _i++) {
        auto i = unique_keys.map(_i);
        if (!embeds[i]) {
    ...
    //         const cache_key_t arg1 = *static_cast<const cache_key_t*>(x);
    //         const cache_key_t arg2 = *static_cast<const cache_key_t*>(y);
    ...
    auto evict_iter = evict.begin();
    auto push_keys_iter = push_keys.begin();
    for (size_t i = 0; i < unique_keys.size(); i++) {
        while (evict_iter != evict.end()
            && (*evict_iter)->key() < embeds[i]->key())
            should_push.push_back(*evict_iter++);

        while (push_keys_iter != push_keys.end()
            && *push_keys_iter < embeds[i]->key())
            push_keys_iter++;

        if (push_keys_iter != push_keys.end() && *push_keys_iter == embeds[i]->key() && embeds[i]->data()) {
            should_push.push_back(embeds[i]);
            shoud_push_indexes.push_back(i);
        }
    }
    while (evict_iter != evict.end())
        should_push.push_back(*evict_iter++);
    ...
    // After push do some clean up
    auto trans_time = std::chrono::system_clock::now();
    for (size_t i = 0, j = 0; i < unique_keys.size(); i++) {
        embeds[i]->setVersion(embeds[i]->getVersion()
                                  + embeds[i]->getUpdates());
        if (j < shoud_push_indexes.size() && shoud_push_indexes[j] == i) {
            embeds[i]->zeroGrad();
            j++;
        }
    }
