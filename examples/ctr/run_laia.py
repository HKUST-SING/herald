...
def worker(args):
    ...
    if dataset == "criteo":
        ...
        if args.all:
            ...
            dense, sparse, labels = process_all_criteo_search_data(return_val=args.val)
        else:
    ...
    if isinstance(dense, tuple):
        sched = htlaia.LAIAScheduler(sparse[0], batch_size)
        dataset_num = 2
        ...
        sched = htlaia.LAIAScheduler(sparse, batch_size)
        dataset_num = 2
        sparse_input = htlaia.laia_dataloader_op(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rdma", action="store_true", help="use RDMA ibverbs to do communication"
    )
    args = parser.parse_args()
    import models

    # setup NCCL ENV parameters
    if not args.rdma:
        print("Use TCP Mode")
        ...
        ib_port = subprocess.check_output("ibdev2netdev").decode()
        ib_port = ib_port.strip("\n").split(" ")[0]
        if "mlx" not in ib_port:
            sys.stderr.write("Cannot find a valid IB port\n")
            sys.exit(-1)
        os.environ["NCCL_IB_HCA"] = ib_port
        sys.stderr.write(
            "{} uses IB: {}".format(
                subprocess.check_output("hostname").decode(), ib_port
            )
        )
