...
def emb_sum_dcn_criteosearch(dense_input, sparse_input, y_, embeding_size=128, enable_push_index=False):
    ...
    sparse_input = ht.embedding_lookup_op(
        Embedding, sparse_input, enable_push_index=enable_push_index, ctx=ht.cpu(0))
    sparse_input = ht.reduce_sum_op(sparse_input, axes=1)
    x = ht.concat_op(sparse_input, dense_input, axis=1)
    # Cross Network
    cross_output = build_cross_layer(x, 1, dense_feature_num, embedding_size, num_layers=3)

    # DNN
    flatten = x
    W1 = init.random_normal(
        [embedding_size + dense_feature_num, 64], stddev=0.001, name="W1")
    W2 = init.random_normal([64, 32], stddev=0.001, name="W2")
    W3 = init.random_normal([32, 16], stddev=0.001, name="W3")

    W4 = init.random_normal(
        [16 + embedding_size + dense_feature_num, 1], stddev=0.001, name="W4")
