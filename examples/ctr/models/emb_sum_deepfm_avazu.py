...
def emb_sum_dfm_avazu(dense_input, sparse_input, y_, embedding_size = 128, enable_push_index=False):
    ...
    sparse_1dim_input = ht.embedding_lookup_op(
        Embedding1, sparse_input, enable_push_index=enable_push_index, ctx=ht.cpu(0))
    ...
    sparse_2dim_input = ht.embedding_lookup_op(
        Embedding2, sparse_input, enable_push_index=enable_push_index, ctx=ht.cpu(0))
    ...
    # DNN
    flatten = ht.array_reshape_op(sparse_2dim_sum, (-1, embedding_size))
    W1 = init.random_normal([embedding_size, 256], stddev=0.01, name="W1")
