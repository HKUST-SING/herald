...
def dfm_avazu(dense_input, sparse_input, y_, partitioner=None, part_all=True, param_on_gpu=True):
    feature_dimension = 33762577 # 33762577
    embedding_size = 128
    ...
    with tf.compat.v1.variable_scope('dfm', dtype=tf.float32, initializer=tf.random_normal_initializer(stddev=0.01), partitioner=all_partitioner):
        ...
        with tf.device(device):
            FM_W = tf.compat.v1.get_variable(name='FM_W', shape=[4, 1])
            W1 = tf.compat.v1.get_variable(
                name='W1', shape=[18*embedding_size, 256])
        ...
        with tf.device('/gpu:0'):
            ...
            # DNN
            flatten = tf.reshape(sparse_2dim_input, (-1, 18*embedding_size))
