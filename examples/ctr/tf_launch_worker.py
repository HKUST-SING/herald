...
def train_large(model, dataset, cluster, task_id, nrank, args):
    ...
    if args.all:
        if dataset == 'movie':
            ...
            trainData, testData = process_all_movie_data()
            trainUsers = get_current_shard(trainData['user_input']).reshape(-1,1)
            trainItems = get_current_shard(trainData['item_input']).reshape(-1,1)
            labels = get_current_shard(trainData['labels']).reshape(-1,1)
            testData = get_current_shard(testData)
            testUserInput = np.repeat(
                np.arange(testData.shape[0], dtype=np.int32), 100).reshape(-1,1)
            testItemInput = testData.reshape((-1,)).reshape(-1,1)
            sparse_feature = np.concatenate([trainUsers, trainItems], 1)
            val_sparse = np.concatenate([testUserInput, testItemInput], 1)
        else:
            if dataset == 'criteo':
                ...
                dense, sparse, all_labels = process_all_avazu_data()
            elif dataset == 'criteosearch':
                from models.load_data import process_all_criteo_search_data
                dense, sparse, all_labels = process_all_criteo_search_data()
    ...
    with tf.device(worker_device):
        if dataset != 'movie':
            dense_input = tf.compat.v1.placeholder(tf.float32, [batch_size, dense_dim])
        sparse_input = tf.compat.v1.placeholder(tf.int32, [batch_size, sparse_dim])
        y_ = y_ = tf.compat.v1.placeholder(tf.float32, [batch_size, 1])

    with tf.compat.v1.device(tf.compat.v1.train.replica_device_setter(cluster=cluster)):
        server_num = len(cluster.as_dict()['ps'])
        embed_partitioner = tf.fixed_size_partitioner(
            server_num, 0) if server_num > 1 else None
        if dataset == 'movie':
            loss, y, opt = model(sparse_input, y_,
    ...
    if dataset == 'movie':
        my_feed_dict = {
            sparse_input: np.empty(shape=(batch_size, sparse_dim)),
            y_: np.empty(shape=(batch_size, 1)),
        }
    else:
        my_feed_dict = {
            dense_input: np.empty(shape=(batch_size, dense_dim)),
            sparse_input: np.empty(shape=(batch_size, sparse_dim)),
            y_: np.empty(shape=(batch_size, 1)),
        }
    ...
    if args.all:
        ...
        iterations = sparse_feature.shape[0] // batch_size
        total_epoch = 1
        start_index = 0
        for ep in range(total_epoch):
            ...
            for it in range(iterations // 10 + (ep % 10 == 9) * (iterations % 10)):
                ...
                if start_index + batch_size > sparse_feature.shape[0]:
                    start_index = 0
        ...
        # here no val
        iteration = sparse_feature.shape[0] // batch_size
...
def main():
    ...
    if raw_model != 'band':
        ...
        if dataset != 'adult':
            train_large(model, dataset, cluster, task_id, len(config['worker']), args)
        elif dataset == 'adult':
