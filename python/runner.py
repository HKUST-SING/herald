...
def start_remote_server(host, local_server_num, identify_file):
    with ssh_connect(host, identify_file) as ssh:
        ...
        stdin, stdout, stderr = ssh.exec_command(
            "PYTHONPATH={}/ PS_VERBOSE=1 DMLC_INTERFACE=eno0 python3 -m hetu.launcher /tmp/hetu_ps_config.yml -n {} > /root/server.log 2>&1".format(
                current_dir, local_server_num
            )
        )
...
def main():
    ...
    if len(settings.hosts) == 1:
        ...
        for proc in _procs:
            proc.start()
        mpi_command = (
            "mpirun --allow-run-as-root --tag-output --bind-to socket --mca btl tcp,self -np %d %s"
            % (settings.num_workers, " ".join(args.command))
        )
        env = dict(os.environ)
        if settings.enable_PS:
            env["DMLC_ROLE"] = "worker"
        print("Run command: %s" % mpi_command)
        ...
        basic_args = "--allow-run-as-root --tag-output --mca btl tcp,self"
        ...
        # redirect stdout
        # command = args.command + [' > /herald/log/worker-$HOSTNAME.txt']
        ...
        print("### starting MPI command: {}".format(mpi_command))
        executor_shell = subprocess.Popen(
