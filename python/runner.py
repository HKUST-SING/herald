...
def start_sched():
    ...
    if USE_RDMA:
        os.environ["DMLC_PS_VAN_TYPE"] = "ibverbs"
...
def ssh_connect(host, identify_file):
    try:
        ssh_directory = (
            ...
            if identify_file == ""
            else os.path.dirname(os.path.abspath(os.path.expanduser(identify_file)))
        )
...
def start_remote_server(host, local_server_num, identify_file):
    with ssh_connect(host, identify_file) as ssh:
        sftp = ssh.open_sftp()
        sftp.put("/tmp/hetu_ps_config.yml", "/tmp/hetu_ps_config.yml", confirm=True)
        sftp.close()
        remote_dmlc_config = "DMLC_INTERFACE={} DMLC_PS_VAN_TYPE={}".format(
            "eno0",
            "ibverbs" if USE_RDMA else "p3"
            # "eno0", "ibverbs"
        )
        print(colored("Start remote server", "red"))
        remote_dmlc_config += " PS_VERBOSE={}".format(PS_LOG_LEVEL)
        stdin, stdout, stderr = ssh.exec_command(
            "PYTHONPATH={}/ {} python3 -m hetu.launcher /tmp/hetu_ps_config.yml -n {} > /herald/tmp/server.log 2>&1".format(
                current_dir, remote_dmlc_config, local_server_num
            )
        )
...
def get_subnet(local_address, remote_hostnames, identify_file=""):
    ...
    config = paramiko.config.SSHConfig.from_path(os.path.join(ssh_directory, "config"))
    remote_address = [
        config.lookup(hostname)["hostname"] for hostname in remote_hostnames
    ]
    remote_address.append(local_address)
    address_pool = set()
    for addr in remote_address:
        binary_repr = int(
            "".join([format(int(part), "08b") for part in addr.split(".")]), 2
        )
    ...
    commonAddress = address_pool[0][:longestCommonPrefix] + "0" * (
        32 - longestCommonPrefix
    )
    parts = [
        commonAddress[:8],
        commonAddress[8:16],
    ...
    subnet = (
        ".".join([str(int(part, 2)) for part in parts]) + "/%d" % longestCommonPrefix
    )
    return subnet
...
def main():
    ...
    parser.add_argument("-i", "--identify", default="", help="SSH identify file.")
    parser.add_argument("-r", "--rdma", action="store_true", help="Use RDMA support")
    parser.add_argument(
    ...
    if len(settings.hosts) == 1:
        ...
        for proc in _procs:
            proc.start()
        mpi_command = (
            "mpirun --allow-run-as-root --tag-output --bind-to socket -np %d %s"
        ...
        subnet = get_subnet(
            settings.chief_address,
            set(settings.hosts) - {settings.chief},
            args.identify,
        )
        if settings.enable_PS:
            ...
            if node == settings.chief:
                ...
                if settings.servers.get(node, 0):
                    proc = multiprocessing.Process(
                        ...
                        args=[node, settings.servers[node], args.identify],
                    )
                    _procs.append(proc)
        # start worker
        for proc in _procs:
            proc.start()
        # basic_args = '--allow-run-as-root --tag-output --bind-to socket'
        # btl_params = "openib,self,vader" if args.rdma else "tcp,self"
        btl_params = "tcp,self"
        basic_args = "--allow-run-as-root --tag-output --mca btl {}".format(btl_params)
        ...
        mpi_ssh_args = (
            ""
            if args.identify == ""
            else "-bootstrap=ssh -bootstrap-exec-args -i %s" % args.identify
        )
        # tcp_intf_arg = '-mca btl_tcp_if_include %s' % subnet
        # tcp_intf_arg += " -mca orte_base_help_aggregate 1"
        # tcp_intf_arg = '-mca btl_tcp_if_include %s' % joined_nics
        env_list = (
            " ".join(
                ["-x %s=%s" % (k, str(v)) for k, v in ps_config.items()]
                + ["-x DMLC_ROLE=worker"]
            )
            if settings.enable_PS
            else ""
        ...
        if args.rdma:
            ...
            tcp_intf_arg = "-mca orte_base_help_aggregate 1"
            tcp_intf_arg += " -mca btl_tcp_if_include %s" % "10.0.0.0/8"
            env_list += " -x DMLC_PS_VAN_TYPE=ibverbs"
        else:
            nccl_socket_intf_arg = "-x NCCL_SOCKET_IFNAME=eno0 -x NCCL_IB_DISABLE=1"
            tcp_intf_arg = "-mca btl_tcp_if_include %s" % "10.0.0.0/8"
            env_list += " -x DMLC_PS_VAN_TYPE=p3"

        # redirect stdout
        command = args.command
        # command += [" > /tmp/worker.log 2>&1"]
        print(command)
        mpi_command = (
            "mpirun {basic_args} "
            "--host {hosts} "
        ...
        print("### starting MPI command: {}\n".format(colored(mpi_command, "blue")))
        executor_shell = subprocess.Popen(
