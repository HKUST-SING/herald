...
class DLContext(ctypes.Structure):
    ...
    def __init__(self, device_id, device_type, hostname='localhost'):
        ...
        #TODO: how to determine in localhost
...
class NDArray(object):
    ...
    def async_h2d(self, source_array, stream_handle, event_handle=None):
        if isinstance(source_array, tuple):
...
class IndexedSlices(object):
    __slots__ = ["indices", "values", "dense_shape", "deduplicated", "lazy", "to_dense_flag", "push_indices"]

    def __init__(self, indices=None, values=None, dense_shape=None, push_indices=None):
    ...
    def update(self, indices, values, dense_shape, push_indices=None):
        self.indices = indices
        self.push_indices = push_indices
        self.values = values
    ...
    def deduplicate(self, stream):
        ...
        if self.push_indices is not None:
            np_push_indices = self.push_indices.asnumpy()
            unique_push_indices = np.unique(np_push_indices)
            push_indices_on_ctx = array(unique_push_indices, ctx=self.push_indices.ctx)
            self.push_indices = push_indices_on_ctx
    ...
    def cpu_deduplicate(self):
        ...
        if self.push_indices is not None:
            np_push_indices = self.push_indices.asnumpy()
            unique_push_indices = np.unique(np_push_indices)
            self.push_indices = array(unique_push_indices, cpu(0))
    ...
    def free_deduplicate(self):
        if self.deduplicated:
            del self.indices
            if self.push_indices is not None:
                del self.push_indices
            del self.values
            self.indices = None
            self.push_indices = None
            self.values = None
            self.deduplicated = False
