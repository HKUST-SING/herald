...
class Dataloader(object):
    def __init__(self, raw_data, batch_size, name='default', func=None, drop_last=True):
        ...
        self.name = str(name)
        assert drop_last, "drop_last must be True"
    ...
    def init_states(self, rank=None, nrank=None):
        if rank is not None:
            ...
            self.raw_data = self.raw_data[range(rank, cur_size*nrank, nrank)]
        ...
        if not self.drop_last:
            ...
            if res_num > 0:
                ...
                # self.arrs.append(np.empty(
                #     tuple([res_num] + list(self.shape[1:]))))
...
class DataloaderWithPushIndex(Dataloader):
    def __init__(self, raw_data, batch_size, name='default', func=None, drop_last=True):
        super().__init__(raw_data, batch_size, name, func, drop_last)
        # self.func = func if func else lambda x: x
        # self.raw_data = np.array(self.func(raw_data), np.uint64)
    ...
    def _get_arr(self, batchind):
        ...
        if type(res) is np.ndarray:
            push_index = {i for i in res.view().reshape(-1)}
        elif type(res) is ndarray.NDArray:
            push_index = {i for i in res.asnumpy().view().reshape(-1)}
        else:
            raise TypeError
        ...
        if batchind == self.max_key:
            ...
            if next_index <= self.samples_num:
                ...
                self.arr_map[self.max_key] = self.queue_size
            self.index = next_index
        return (res, np.array(list(push_index), dtype=np.uint64))

    def get_arr(self):
        # step forward in this function
        res = self._get_arr(self.batch_index)
        self.last_batch_size = res[0].shape[0]
        self.batch_index = (self.batch_index + 1) % self.samples_num
        return res

def dataloader_with_push_index_op(dataloaders):
    '''
    dataloaders: list of dataloaders
    '''
    temp_dataloaders = []
    for dl in dataloaders:
        if isinstance(dl, DataloaderWithPushIndex):
            temp_dataloaders.append(dl)
        elif isinstance(dl, list):
            temp_dataloaders.append(DataloaderWithPushIndex(*dl))
        elif isinstance(dl, dict):
            temp_dataloaders.append(DataloaderWithPushIndex(**dl))
        else:
            assert False, 'Dataloader parameter invalid.'
