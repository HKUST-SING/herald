...
class HetuConfig(object):
    ...
    def __init__(
        ...
        enable_lazy=True,
        cache_bound=100,
        cache_limit=int(1.6*1024*1024*1024*0.1/(4*128)),
        cache_perf_enable=False,
        log_path=None,
        pipeline=None,
        ...
        self.cache_perf_enable = cache_perf_enable
...
class Executor(object):
    ...
    def __init__(self, eval_node_dict, config=None, sched=None, sched_params=None, **kargs):
        """
        Parameters
        ----------
        ...
        if sched is not None:
            assert sched_params is not None, "sched_params cannot be None when sched is not None."
            assert isinstance(sched_params, dict), 'sched_params type %s invalid.' % str(type(sched_params))
            sched_params["config"] = config
            sched.start(**sched_params)

        topo_sort_with_hook(config.my_eval_nodes, config)
