...
class ParameterServerCommunicateOp(Op):
    ...
    def _push_cache(self, input_val, stream_handle):
        if input_val.push_indices is None:
            return self.cache.embedding_update(input_val.indices, input_val.values)
        else:
            return self.cache.embedding_update_with_push_keys(input_val.indices, input_val.push_indices, input_val.values)
    ...
    def forward_hook(self, config):
        ...
        self.use_cache_table = config.cstable_policy is not None and self.parameter.is_embed
        # using cache
        ...
        if self.use_cache_table:
            ...
            # ALREADY has barrier!
            # print("Create Sparse Tab with limit ({}), length ({}), ".format(config.cache_limit, node_shape[0]) +
            #         "width ({}), node_id ({}), ".format(node_shape[1], self.parameter.id) + 
            #         "policy ({}), bound ({})".format(config.cstable_policy, config.cache_bound))
            self.cache = CacheSparseTable(
                config.cache_limit, node_shape[0], node_shape[1], self.parameter.id, config.cstable_policy, config.cache_bound)
            # NOTE: enbale perf will harm 
            self.cache.perf_enabled(config.cache_perf_enable)
    ...
    def get_cache(self):
        # Not necessary use cache, return None for no cache case
        if self.use_cache_table:
            return self.cache
        else:
            return None
